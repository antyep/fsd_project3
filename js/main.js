...
window.addEventListener("DOMContentLoaded", () => {
	...
	const getCurrentSelection = () => JSON.parse(sessionStorage.getItem(SESSION_KEYS.CURRENT_SELECTION)) || {};
	const getCurrentDifficulty = () => sessionStorage.getItem(SESSION_KEYS.DIFFICULTY);
	const getCurrentTries = () => JSON.parse(sessionStorage.getItem(SESSION_KEYS.CURRENT_TRIES)) || [];
	const getCurrentEditingColor = () => sessionStorage.getItem(SESSION_KEYS.CURRENT_EDITING_COLOR)
	const getSecretCombination = () => JSON.parse(sessionStorage.getItem(SESSION_KEYS.SECRET_COMBINATION))
	...
	const clearSelection = () => {
		...
		const currentColorContainer = document.querySelector('#currentColorContainer');
		if (currentColorContainer) {
			const colorPickers = currentColorContainer.querySelectorAll('.colorPicker')
	...
	const checkCombination = () => {
		const currentTries = getCurrentTries()
		const secretCombination = getSecretCombination()
		const lastCombination = Object.values(currentTries[currentTries.length - 1])
	...
	const setupLevelIndicator = () => {
		...
		if (currentLevelContainer) currentLevelContainer.innerHTML = getCurrentDifficulty()
	}
	...
	const setupCurrentColors = () => {
		...
		if (currentColorContainer) {
			...
			for (let i = 0; i < (maxLength); i++) {
				...
				if (currentSelection[i]) colorPickerElem.style.backgroundColor = currentSelection[i]
	...
	const setupDifficultiesScreen = () => {
		const difficultiesContainer = document.querySelector('#difficultiesContainer');
		if (difficultiesContainer) {
			for (const difficulty in DIFFICULTY) {
				const difficultyElem = document.createElement("button");
				difficultyElem.innerHTML = difficulty.charAt(0).toUpperCase() + difficulty.slice(1);
		...
		if (colorPickerContainer) {
			...
			for (let i = 0; i < (maxLength); i++) {
				...
				colorPickerElem.addEventListener('click', () => {
					...
					sessionStorage.setItem(SESSION_KEYS.CURRENT_SELECTION, JSON.stringify(CURRENT_SELECTION))
					window.location.href = './game.html'
				})
				// Adding element we created inside the container
